"""Builds samples/side.html — portrait (3:4) stock card with the photo and name in the left
two-thirds and the specs in the right third, laid out on a rule-of-thirds grid.

Card: 503.91 x 671.88 (3:4)  ->  columns every 167.97, rows every 223.96.
...
EXTRA_CSS = """
  .band { height: 880px; }
  .track { padding-top: 104px; }

  .side { width: 503.91px; height: 671.88px; display: grid; grid-template-columns: 335.94px 167.97px; background: var(--black); color: var(--bone);
          transition: transform .3s ease, box-shadow .3s ease; }
  .side:hover { transform: translateY(-8px); box-shadow: 0 20px 40px rgba(11, 10, 7, .25); }

  /* Left two-thirds: photo as large as possible, name on the lower third line */
  ...
  .side .photo::after { content: ""; position: absolute; left: 0; right: 0; bottom: 0; height: 50%; background: linear-gradient(to bottom, rgba(11, 10, 7, 0), rgba(11, 10, 7, .88)); }
  .side .title { position: absolute; left: 28px; right: 20px; top: 447.92px; bottom: 0; z-index: 1; display: flex; flex-direction: column; justify-content: center; }
  .side .layout { display: flex; align-items: center; gap: 12px; font: 600 17px/1 var(--poppins); letter-spacing: .14em; text-transform: uppercase; color: var(--bone); }
  .side .layout::before { content: ""; width: 12px; height: 12px; background: var(--green); transform: rotate(45deg); }
  .side h3 { margin-top: 12px; font: 700 66px/1 var(--oswald); letter-spacing: .01em; color: #fff; }
  .side .badge { top: 24px; right: auto; left: 0; padding: 10px 38px 10px 20px; clip-path: polygon(0 0, 100% 0, calc(100% - 20px) 50%, 100% 100%, 0 100%); }

  /* Right third: specs, one thirds-row each — ATM, then Tare, then Sleeps + Length */
  .side .rail { margin: 0; display: grid; grid-template-rows: repeat(3, 223.96px); }
  .side .row { padding: 0 16px 0 20px; display: flex; flex-direction: column; justify-content: center; gap: 20px; }
  .side .row + .row { border-top: 1px solid rgba(244, 241, 234, .16); }
  .side dt { font: 400 13px/1 var(--poppins); letter-spacing: .12em; text-transform: uppercase; color: var(--silver); }
  .side dd { margin: 8px 0 0; font-family: var(--oswald); font-weight: 500; line-height: 1; white-space: nowrap; color: var(--bone); }
  .side dd small { display: block; margin-top: 6px; font: 500 15px/1 var(--poppins); letter-spacing: .04em; color: var(--silver); }
  .side .atm dt { font-size: 15px; font-weight: 600; color: var(--green); }
  .side .atm dd { font-size: 56px; }
  .side .r1 { background: rgba(139, 155, 77, .12); box-shadow: inset 4px 0 0 var(--green); }
  .side .tare dd { font-size: 40px; }
  .side .tare dd small { font-size: 14px; }
  .side .sleeps dd { font-size: 30px; color: #d9d5cc; }
  .side .length dd { font-size: 24px; color: var(--silver); }
  .side .sleeps dt, .side .length dt { font-size: 12px; }
...
section = f'''
  <section class="sample">
    <div class="intro">
      ...
      <div><h2>Side Specs — Portrait 3:4</h2><p>The photo and name fill the left two-thirds, and the specs take the right third. Each spec row is one third of the card height: ATM, then Tare, then Sleeps and Length.</p></div>
    </div>
    <div class="band">
...
html = html.replace("<title>Drive Away card samples | Outbound RVs</title>", "<title>Side specs card sample | Outbound RVs</title>")
html = html.replace("<h1>DRIVE AWAY — CARD SAMPLES</h1>", "<h1>SIDE SPECS — CARD SAMPLE</h1>")
...
open("side.html", "w").write(html)
print("samples/side.html written")
